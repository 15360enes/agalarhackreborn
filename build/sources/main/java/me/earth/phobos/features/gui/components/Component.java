...
class Component
    ...
    void drawScreen ( int mouseX , int mouseY , float partialTicks ) {
        ...
        if ( this.open ) {
            ...
            if ( ClickGui.getInstance ( ).outline.getValue ( ) ) {
                if ( ClickGui.getInstance ( ).rainbowRolling.getValue ( ) ) {
                    ...
                    GlStateManager.shadeModel ( 7425 );
                    GL11.glBegin ( 2 );
                    Color outlineColor = ClickGui.getInstance ( ).colorSync.getValue ( ) ? new Color ( Colors.INSTANCE.getCurrentColorHex ( ) ) : new Color ( Agalar.colorManager.getColorAsIntFullAlpha ( ) );
        ...
        Agalar.textManager.drawStringWithShadow ( this.getName ( ) , (float) this.x + 3.0f , (float) this.y - 4.0f - (float) PhobosGui.getClickGui ( ).getTextOffset ( ) , - 1 );
        if ( this.open ) {

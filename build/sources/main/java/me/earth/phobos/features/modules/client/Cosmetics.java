...
class Cosmetics
    ...
    void onRenderPlayer ( RenderPlayerEvent.Post event ) {
        if ( Agalar.cosmeticsManager.hasCosmetics ( event.getEntityPlayer ( ) ) || EntityUtil.isFakePlayer ( event.getEntityPlayer ( ) ) ) {
            return;
        }
        for (ModelBase model : Agalar.cosmeticsManager.getRenderModels ( event.getEntityPlayer ( ) )) {

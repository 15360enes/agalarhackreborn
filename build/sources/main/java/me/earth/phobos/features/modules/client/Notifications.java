...
class Notifications
    ...
    private final Timer weak = new Timer ( );
    private final Timer strgh = new Timer ( );
    ...
    private final Setting < Boolean > bold = this.register ( new Setting < Object > ( "BoldMsgs" , Boolean.FALSE , v -> this.moduleMessage.getValue ( ) ) );
    ...
    public Setting < Boolean > strength = this.register ( new Setting <> ( "Strength" , false ) );
    public Setting < Integer > strcheckrate = this.register ( new Setting <> ( "CheckRate" , 500 , 0 , 5000 , v -> this.strength.getValue ( ) ) );
    public Setting < Boolean > weakness = this.register ( new Setting <> ( "Weakness" , false ) );
    public Setting < Boolean > strongness = this.register ( new Setting <> ( "Strongness" , false , v -> this.weakness.getValue ( ) ) );
    public Setting < Integer > checkrate = this.register ( new Setting <> ( "CheckRate" , 500 , 0 , 5000 , v -> this.weakness.getValue ( ) ) );
    public Setting < Boolean > thirtytwokay = this.register ( new Setting <> ( "32k" , false ) );
    public Setting < Boolean > tp = this.register ( new Setting <> ( "Chorus" , false ) );
    ...
    void onUpdate ( ) {
        ...
        if ( this.visualRange.getValue ( ) ) {
            ...
            if ( tickPlayerList.size ( ) > 0 ) {
                for (EntityPlayer player : tickPlayerList) {
                    ...
                    if ( Agalar.friendManager.isFriend ( player ) ) {
        ...
        if ( this.pearls.getValue ( ) ) {
            ...
            if ( closestPlayer != null && flag ) {
                ...
                Command.sendSilentMessage ( Agalar.friendManager.isFriend ( closestPlayer.getName ( ) ) ? ( ChatFormatting.AQUA + closestPlayer.getName ( ) + ChatFormatting.DARK_GRAY + " has just thrown a pearl heading " + facing + "!" ) : ( ChatFormatting.RED + closestPlayer.getName ( ) + ChatFormatting.DARK_GRAY + " has just thrown a pearl heading " + facing + "!" ) );
                flag = false;
            }
        }
        if ( this.weakness.getValue ( ) ) { // TODO: 7/23/2021 fix for pingbypass 
            if ( ! this.weak.passedMs ( this.checkrate.getValue ( ) ) ) return;
            if ( Notifications.mc.player.isPotionActive ( Objects.requireNonNull ( Potion.getPotionFromResourceLocation ( "weakness" ) ) ) && ! this.last ) {
        ...
        if ( this.strength.getValue ( ) ) {
            for (EntityPlayer entityPlayer : Notifications.mc.world.playerEntities) {
                if ( ! this.strgh.passedMs ( this.strcheckrate.getValue ( ) ) ) return;
                if ( entityPlayer.equals ( Notifications.mc.player ) ) continue;
                ...
                if ( ! this.str.contains ( entityPlayer ) || entityPlayer.isPotionActive ( MobEffects.STRENGTH ) )
                    continue;
                Command.sendMessage ( entityPlayer.getDisplayNameString ( ) + ChatFormatting.GREEN + " doesnt have strength" );
    ...
    public
    void onTick ( ) {
        if ( Notifications.fullNullCheck ( ) ) return;
    ...
    void onToggleModule ( ClientEvent event ) {
        ...
        if ( ! ( event.getStage ( ) != 0 || ( module = (Module) event.getFeature ( ) ).equals ( this ) || ! modules.contains ( module.getDisplayName ( ) ) && this.list.getValue ( ) ) ) {
            ...
            if ( this.watermark.getValue ( ) ) {
                TextComponentString textComponentString = new TextComponentString ( Agalar.commandManager.getClientMessage ( ) + " " + "\u00a7r" + "\u00a7c" + module.getDisplayName ( ) + " disabled." );
                ...
                if ( this.bold.getValue ( ) ) {
                    TextComponentString textComponentString2 = new TextComponentString ( Agalar.commandManager.getClientMessage ( ) + " " + ChatFormatting.BOLD + module.getDisplayName ( ) + ChatFormatting.RED + " disabled." );
                    Notifications.mc.ingameGUI.getChatGUI ( ).printChatMessageWithOptionalDeletion ( textComponentString2 , moduleNumber );
                }
            } else {
        ...
        if ( event.getStage ( ) == 1 && ( modules.contains ( ( module = (Module) event.getFeature ( ) ).getDisplayName ( ) ) || ! this.list.getValue ( ) ) ) {
            ...
            if ( this.watermark.getValue ( ) ) {
                TextComponentString textComponentString = new TextComponentString ( Agalar.commandManager.getClientMessage ( ) + " " + "\u00a7r" + "\u00a7a" + module.getDisplayName ( ) + " enabled." );
                Notifications.mc.ingameGUI.getChatGUI ( ).printChatMessageWithOptionalDeletion ( textComponentString , moduleNumber );
                if ( this.bold.getValue ( ) ) {
                    TextComponentString textComponentString2 = new TextComponentString ( Agalar.commandManager.getClientMessage ( ) + " " + ChatFormatting.BOLD + module.getDisplayName ( ) + ChatFormatting.GREEN + " enabled." );
                    Notifications.mc.ingameGUI.getChatGUI ( ).printChatMessageWithOptionalDeletion ( textComponentString2 , moduleNumber );
                }
            } else {

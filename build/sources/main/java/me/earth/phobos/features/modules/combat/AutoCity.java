...
class AutoCity
    ...
    ArrayList < PairUtil < EntityPlayer, ArrayList < BlockPos > > > GetPlayersReadyToBeCitied ( ) {
        ...
        for (EntityPlayer entity : Objects.requireNonNull ( EntityUtil.getNearbyPlayers ( 6.0 ) ).stream ( ).filter ( entityPlayer -> ! Agalar.friendManager.isFriend ( entityPlayer ) ).collect ( Collectors.toList ( ) )) {
    ...
    void onUpdateWalkingPlayer ( UpdateWalkingPlayerEvent updateWalkingPlayerEvent ) {
        ...
        if ( this.rotate.getValue ( ) ) {
            ...
            Agalar.rotationManager.lookAtPos ( blockPos );

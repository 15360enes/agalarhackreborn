...
class BowSpam
    ...
    void onUpdateWalkingPlayer ( UpdateWalkingPlayerEvent event ) {
        ...
        if ( this.autoSwitch.getValue ( ) && InventoryUtil.findHotbarBlock ( ItemBow.class ) != - 1 && this.ownHealth.getValue ( ) <= EntityUtil.getHealth ( BowSpam.mc.player ) && ( ! this.onlyWhenSave.getValue ( ) || EntityUtil.isSafe ( BowSpam.mc.player ) ) ) {
            ...
            if ( ! ( target == null || Agalar.moduleManager.getModuleByClass ( AutoCrystal.class ).isOn ( ) && InventoryUtil.holdingItem ( ItemEndCrystal.class ) ) ) {
                ...
                Agalar.rotationManager.lookAtVec3d ( xPos , yPos , zPos );
                if ( BowSpam.mc.player.getHeldItemMainhand ( ).getItem ( ) instanceof ItemBow ) {
                    this.switched = true;
                }
            }
        ...
        if ( this.mode.getValue ( ) == Mode.FAST && ( this.offhand || BowSpam.mc.player.inventory.getCurrentItem ( ).getItem ( ) instanceof ItemBow ) && BowSpam.mc.player.isHandActive ( ) ) {
            ...
            float f3 = this.tpsSync.getValue ( ) ? Agalar.serverManager.getTpsFactor ( ) : 1.0f;
            if ( f >= f2 * f3 ) {
    ...
    void onUpdate ( ) {
        ...
        switch (this.mode.getValue ( )) {
            case AUTORELEASE: {
                if ( ! this.offhand && ! ( BowSpam.mc.player.inventory.getCurrentItem ( ).getItem ( ) instanceof ItemBow ) || ! this.timer.passedMs ( (int) ( (float) this.delay.getValue ( ) * ( this.tpsSync.getValue ( ) ? Agalar.serverManager.getTpsFactor ( ) : 1.0f ) ) ) )
                    break;
    ...
    EntityPlayer getTarget ( ) {
        ...
        for (EntityPlayer player : BowSpam.mc.world.playerEntities) {
            if ( player == null || EntityUtil.isDead ( player ) || EntityUtil.getHealth ( player ) > this.health.getValue ( ) || player.equals ( BowSpam.mc.player ) || Agalar.friendManager.isFriend ( player ) || BowSpam.mc.player.getDistanceSq ( player ) > MathUtil.square ( this.range.getValue ( ) ) || ! BowSpam.mc.player.canEntityBeSeen ( player ) && ! EntityUtil.canEntityFeetBeSeen ( player ) )
                continue;
            if ( target == null ) {
                target = player;

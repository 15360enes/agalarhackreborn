...
class Surround
    ...
    void onEnable ( ) {
        ...
        if ( this.center.getValue ( ) && ! Agalar.moduleManager.isModuleEnabled ( "Freecam" ) ) {
            if ( Surround.mc.world.getBlockState ( new BlockPos ( Surround.mc.player.getPositionVector ( ) ) ).getBlock ( ) == Blocks.WEB ) {
                Agalar.positionManager.setPositionPacket ( Surround.mc.player.posX , this.startPos.getY ( ) , Surround.mc.player.posZ , true , true , true );
            } else {
                Agalar.positionManager.setPositionPacket ( (double) this.startPos.getX ( ) + 0.5 , this.startPos.getY ( ) , (double) this.startPos.getZ ( ) + 0.5 , true , true , true );
            }
        }
    ...
    void doFeetPlace ( ) {
        ...
        if ( this.blockface.getValue ( ) ) {
            this.isSafe = 0;
            this.placeBlocks ( Surround.mc.player.getPositionVector ( ).add ( 0 , 0.125 , 0 ) , EntityUtil.getUnsafeBlockArray ( Surround.mc.player , 1 , this.floor.getValue ( ) , true ) , this.helpingBlocks.getValue ( ) , false , false );
        }
        if ( ! EntityUtil.isSafe ( Surround.mc.player , 0 , this.floor.getValue ( ) , true ) ) {
            this.isSafe = 0;
            this.placeBlocks ( Surround.mc.player.getPositionVector ( ).add ( 0 , 0.125 , 0 ) , EntityUtil.getUnsafeBlockArray ( Surround.mc.player , 0 , this.floor.getValue ( ) , true ) , this.helpingBlocks.getValue ( ) , false , false );
        } else if ( ! EntityUtil.isSafe ( Surround.mc.player , - 1 , false , true ) ) {
            this.isSafe = 1;
            if ( this.antiPedo.getValue ( ) ) {
                this.placeBlocks ( Surround.mc.player.getPositionVector ( ).add ( 0 , 0.125 , 0 ) , EntityUtil.getUnsafeBlockArray ( Surround.mc.player , - 1 , false , true ) , false , false , true );
            }
        } else {
            this.isSafe = 2;
        }
    ...
    Vec3d areClose ( Vec3d[] vec3ds ) {
        ...
        if ( matches == 2 ) {
            return Surround.mc.player.getPositionVector ( ).add ( vec3ds[0].add ( vec3ds[1] ) ).add ( 0 , 0.130 , 0 );
        }
        return null;
    }
        ...
        for (Vec3d vec3d : vec3ds) {
            ...
            switch (BlockUtil.isPositionPlaceable ( position , this.raytrace.getValue ( ) )) {
                ...
                case 1: {
                    ...
                    if ( ! this.extendMove.getValue ( ) && Agalar.speedManager.getSpeedKpH ( ) != 0.0 || isExtending || this.extenders >= this.extender.getValue ( ) )
                        continue block6;
                    this.placeBlocks ( Surround.mc.player.getPositionVector ( ).add ( 0 , 0.125 , 0 ).add ( vec3d ) , EntityUtil.getUnsafeBlockArrayFromVec3d ( Surround.mc.player.getPositionVector ( ).add ( 0 , 0.125 , 0 ).add ( vec3d ) , 0 , this.floor.getValue ( ) , true ) , hasHelpingBlocks , false , true );
    ...
    boolean check ( ) {
        ...
        switch (this.movementMode.getValue ( )) {
            ...
            case LIMIT: {
                if ( ! ( Agalar.speedManager.getSpeedKpH ( ) > this.speed.getValue ( ) ) ) break;
                return true;
            }
            case OFF: {
                if ( ! ( Agalar.speedManager.getSpeedKpH ( ) > this.speed.getValue ( ) ) ) break;
        ...
        return Agalar.moduleManager.isModuleEnabled ( "Freecam" ) || ! this.timer.passedMs ( this.delay.getValue ( ) ) || this.switchMode.getValue ( ) == InventoryUtil.Switch.NONE && Surround.mc.player.inventory.currentItem != InventoryUtil.findHotbarBlock ( BlockObsidian.class );
    }
    ...
    List < BlockPos > fuckYou3arthqu4keYourCodeIsGarbage ( ) {
        if ( this.floor.getValue ( ) ) {
            return Arrays.asList ( new BlockPos ( Surround.mc.player.getPositionVector ( ) ).add ( 0 , - 1 + 0.125 , 0 ) , new BlockPos ( Surround.mc.player.getPositionVector ( ) ).add ( 1 , 0 + 0.125 , 0 ) , new BlockPos ( Surround.mc.player.getPositionVector ( ) ).add ( - 1 , 0 + 0.125 , 0 ) , new BlockPos ( Surround.mc.player.getPositionVector ( ) ).add ( 0 , 0 + 0.125 , - 1 ) , new BlockPos ( Surround.mc.player.getPositionVector ( ) ).add ( 0 , 0 + 0.125 , 1 ) );
        }
        return Arrays.asList ( new BlockPos ( Surround.mc.player.getPositionVector ( ) ).add ( 1 , 0 + 0.125 , 0 ) , new BlockPos ( Surround.mc.player.getPositionVector ( ) ).add ( - 1 , 0 + 0.125 , 0 ) , new BlockPos ( Surround.mc.player.getPositionVector ( ) ).add ( 0 , 0 + 0.125 , - 1 ) , new BlockPos ( Surround.mc.player.getPositionVector ( ) ).add ( 0 , 0 + 0.125 , 1 ) ); // fuck you
    }

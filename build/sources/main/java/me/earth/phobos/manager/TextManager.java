...
class TextManager
    ...
    private me.earth.phobos.features.gui.font.CustomFont customFont = new me.earth.phobos.features.gui.font.CustomFont ( new Font ( "Verdana" , Font.PLAIN , 17 ) , true , false );
    ...
    void init ( boolean startup ) {
        CustomFont cFont = Agalar.moduleManager.getModuleByClass ( CustomFont.class );
        try {
    ...
    float drawString ( String text , float x , float y , int color , boolean shadow ) {
        if ( Agalar.moduleManager.isModuleEnabled ( CustomFont.class ) ) {
            if ( shadow ) {

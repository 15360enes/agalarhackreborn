...
public class HUD
    ...
    private final Setting<String> customWatermark = this.register(new Setting<String>("WatermarkName", "Agalar Client"));
    ...
    private final Setting<String> spoofGreeter = this.register(new Setting<Object>("GreeterName", "AgalarClient", v -> this.greeter.getValue() == Greeter.CUSTOM));
    ...
    public void onUpdate() {
        for (Module module : Agalar.moduleManager.sortedModules) {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        switch (this.watermark.getValue()) {
            case AGALAR: {
                this.renderer.drawString(Agalar.MODNAME + (this.modeVer.getValue() != false ? " " + Agalar.MODVER : ""), 2.0f, 2.0f, this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(2) : this.color, true);
                break;
            }
            case DUNYA: {
                this.renderer.drawString(Agalar.MODNAME + (this.modeVer.getValue() != false ? " " + Agalar.MODVER : ""), 2.0f, 2.0f, this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(2) : this.color, true);
            ...
            case CUSTOM: {
                this.renderer.drawString(this.customWatermark.getValue() + (this.modeVer.getValue() != false ? " " + Agalar.MODVER : ""), 2.0f, 2.0f, this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(2) : this.color, true);
            }
        }
        ...
        if (this.arrayList.getValue().booleanValue()) {
            ...
            if (this.renderingUp.getValue().booleanValue()) {
                for (int i = 0; i < (this.alphabeticalSorting.getValue() != false ? Agalar.moduleManager.alphabeticallySortedModules.size() : Agalar.moduleManager.sortedModules.size()); ++i) {
                    module = this.alphabeticalSorting.getValue() != false ? Agalar.moduleManager.alphabeticallySortedModules.get(i) : Agalar.moduleManager.sortedModules.get(i);
                    text3 = module.getDisplayName() + "\u00a77" + (module.getDisplayInfo() != null ? " [\u00a7f" + module.getDisplayInfo() + "\u00a77" + "]" : "");
                    moduleColor = Agalar.moduleManager.moduleColorMap.get(module);
                ...
                for (int i = 0; i < (this.alphabeticalSorting.getValue() != false ? Agalar.moduleManager.alphabeticallySortedModules.size() : Agalar.moduleManager.sortedModules.size()); ++i) {
                    module = this.alphabeticalSorting.getValue() != false ? Agalar.moduleManager.alphabeticallySortedModules.get(Agalar.moduleManager.alphabeticallySortedModules.size() - 1 - i) : Agalar.moduleManager.sortedModules.get(i);
                    text3 = module.getDisplayName() + "\u00a77" + (module.getDisplayInfo() != null ? " [\u00a7f" + module.getDisplayInfo() + "\u00a77" + "]" : "");
                    moduleColor = Agalar.moduleManager.moduleColorMap.get(module);
        ...
        if (this.renderingUp.getValue().booleanValue()) {
            ...
            if (this.serverBrand.getValue().booleanValue()) {
                text2 = grayString + "Server brand " + "\u00a7f" + Agalar.serverManager.getServerBrand();
            ...
            if (this.potions.getValue().booleanValue()) {
                for (PotionEffect effect : Agalar.potionManager.getOwnPotions()) {
                    text = this.altPotionsColors.getValue() != false ? Agalar.potionManager.getPotionString(effect) : Agalar.potionManager.getColoredPotionString(effect);
            ...
            if (this.speed.getValue().booleanValue()) {
                text2 = grayString + "Speed " + "\u00a7f" + Agalar.speedManager.getSpeedKpH() + " km/h";
                this.renderer.drawString((String) text2, width - (this.renderer.getStringWidth((String) text2) + 2), height - 2 - (i += 10), this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(height - i) : this.color, true);
            }
            ...
            if (this.tps.getValue().booleanValue()) {
                String text4 = grayString + "TPS " + "\u00a7f" + Agalar.serverManager.getTPS();
            ...
            text3 = grayString + "Ping " + "\u00a7f" + (ServerModule.getInstance().isConnected() ? ServerModule.getInstance().getServerPing() : (long) Agalar.serverManager.getPing()) + (this.MS.getValue() != false ? "ms" : "");
            ...
            if (this.tps.getValue().booleanValue()) {
                String text5 = grayString + "TPS " + "\u00a7f" + Agalar.serverManager.getTPS();
            ...
            fpsText = grayString + "FPS " + "\u00a7f" + Minecraft.debugFPS;
            text3 = grayString + "Ping " + "\u00a7f" + Agalar.serverManager.getPing();
            if (this.renderer.getStringWidth(text3) > this.renderer.getStringWidth(fpsText)) {
                if (this.ping.getValue().booleanValue()) {
                    this.renderer.drawString(text3, width - (this.renderer.getStringWidth(text3) + 2), 2 + i++ * 10, this.rolling.getValue() != false && this.rainbow.getValue() != false ? this.colorMap.get(2 + i * 10) : this.color, true);
                }
        ...
        if (this.renderingUp.getValue().booleanValue()) {
            Agalar.notificationManager.handleNotifications(height - (i + 16));
        } else {
            Agalar.notificationManager.handleNotifications(height - (j + 16));
        }
        i = HUD.mc.currentScreen instanceof GuiChat ? 14 : 0;
        String coordinates = grayString + "XYZ " + "\u00a7f" + posX + ", " + posY + ", " + posZ + " " + "\u00a77" + "[" + "\u00a7f" + hposX + ", " + hposZ + "\u00a77" + "]";
        String text6 = (this.direction.getValue() != false ? Agalar.rotationManager.getDirection4D(false) + " " : "") + (this.coords.getValue() != false ? coordinates : "") + "";
    ...
    public void renderGreeter() {
        ...
        switch (this.greeter.getValue()) {
            ...
            case LONG: {
                text = text + "Welcome to Agalar.eu " + HUD.mc.player.getDisplayNameString() + " :^)";
                break;
            }
            case CUSTOM: {
    ...
    public void renderLag() {
        int width = this.renderer.scaledWidth;
        if (Agalar.serverManager.isServerNotResponding()) {
            String text = (this.lag.getValue() == LagNotify.GRAY ? "\u00a77" : "\u00a7c") + "Server not responding: " + MathUtil.round((float) Agalar.serverManager.serverRespondingTime() / 1000.0f, 1) + "s.";

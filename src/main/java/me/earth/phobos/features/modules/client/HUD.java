...
class HUD
    ...
    private final Setting < String > customWatermark = this.register ( new Setting <> ( "WatermarkName" , "AgalarHack" ) );
    ...
    public
    void onUpdate ( ) {
        for (final Module module : Agalar.moduleManager.sortedModules) {
    ...
    void onRender2D ( final Render2DEvent event ) {
        ...
        if ( this.arrayList.getValue ( ) ) {
            if ( this.renderingUp.getValue ( ) ) {
                for (int k = 0; k < ( this.alphabeticalSorting.getValue ( ) ? Agalar.moduleManager.alphabeticallySortedModules.size ( ) : Agalar.moduleManager.sortedModules.size ( ) ); ++ k) {
                    final Module module = this.alphabeticalSorting.getValue ( ) ? Agalar.moduleManager.alphabeticallySortedModules.get ( k ) : Agalar.moduleManager.sortedModules.get ( k );
                    final String text = module.getDisplayName ( ) + "\u00A77" + ( ( module.getDisplayInfo ( ) != null ) ? ( " [\u00A7f" + module.getDisplayInfo ( ) + "\u00A77" + "]" ) : "" );
                    final Color moduleColor = Agalar.moduleManager.moduleColorMap.get ( module );
                ...
                for (int k = 0; k < ( this.alphabeticalSorting.getValue ( ) ? Agalar.moduleManager.alphabeticallySortedModules.size ( ) : Agalar.moduleManager.sortedModules.size ( ) ); ++ k) {
                    final Module module = this.alphabeticalSorting.getValue ( ) ? Agalar.moduleManager.alphabeticallySortedModules.get ( Agalar.moduleManager.alphabeticallySortedModules.size ( ) - 1 - k ) : Agalar.moduleManager.sortedModules.get ( k );
                    final String text = module.getDisplayName ( ) + "\u00A77" + ( ( module.getDisplayInfo ( ) != null ) ? ( " [\u00A7f" + module.getDisplayInfo ( ) + "\u00A77" + "]" ) : "" );
                    final Color moduleColor = Agalar.moduleManager.moduleColorMap.get ( module );
        ...
        if ( this.renderingUp.getValue ( ) ) {
            ...
            if ( this.potions.getValue ( ) ) {
                for (final PotionEffect effect : Agalar.potionManager.getOwnPotions ( )) {
                    final String text3 = this.altPotionsColors.getValue ( ) ? Agalar.potionManager.getPotionString ( effect ) : Agalar.potionManager.getColoredPotionString ( effect );
        ...
        if ( this.renderingUp.getValue ( ) ) {
            Agalar.notificationManager.handleNotifications ( height - ( k + 16 ) );
        } else {
        ...
        final String text4 = ( this.direction.getValue ( ) ? ( Agalar.rotationManager.getDirection4D ( false ) + " " ) : "" ) + ( this.coords.getValue ( ) ? coordinates : "" ) + "";
    ...
    void renderLag ( ) {
        ...
        if ( Agalar.serverManager.isServerNotResponding ( ) ) {
            final String text = ( ( this.lag.getValue ( ) == LagNotify.GRAY ) ? "\u00A77" : "\u00A7c" ) + "Server not responding: " + MathUtil.round ( Agalar.serverManager.serverRespondingTime ( ) / 1000.0f , 1 ) + "s.";

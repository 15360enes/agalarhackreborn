...
class PhobosChat
    ...
    void onRender3D ( final Render3DEvent event ) {
        ...
        if ( this.waypoints.getValue ( ) ) {
            for (final WaypointManager.Waypoint waypoint : Agalar.waypointManager.waypoints.values ( )) {
    ...
    void onRender2D ( final Render2DEvent event ) {
        ...
        if ( this.inventories.getValue ( ) ) {
            ...
            for (final String player : PhobosChat.phobosUsers) {
                if ( Agalar.inventoryManager.inventories.get ( player ) != null ) {
                    continue;
                }
                final List < ItemStack > stacks = Agalar.inventoryManager.inventories.get ( player );
                            ...
                            if ( input.equalsIgnoreCase ( "list" ) ) {
                                ...
                                for (final String friend : split2) {
                                    if ( ! friend.equals ( Util.mc.player.getName ( ) ) ) {
                                        Agalar.friendManager.addFriend ( friend );
                                ...
                                Agalar.waypointManager.waypoints.put ( name , new WaypointManager.Waypoint ( name , server , Integer.parseInt ( dimension ) , Integer.parseInt ( inputs[2] ) , Integer.parseInt ( inputs[3] ) , Integer.parseInt ( inputs[4] ) , color ) );
                                ...
                                final String name = this.inputStream.readUTF ( );
                                Agalar.waypointManager.waypoints.remove ( name );
                                ...
                                for (final String player : PhobosChat.phobosUsers) {
                                    if ( player.equalsIgnoreCase ( name ) ) {
                                        Agalar.inventoryManager.inventories.put ( player , PhobosChat.deserializeInventory ( inventory ) );
                                    }
                                }

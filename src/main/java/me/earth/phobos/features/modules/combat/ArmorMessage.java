...
public class ArmorMessage
    ...
    public void onUpdate(UpdateWalkingPlayerEvent event) {
        for (EntityPlayer player : ArmorMessage.mc.world.playerEntities) {
            if (player.isDead || !Agalar.friendManager.isFriend(player.getName())) continue;
            for (ItemStack stack : player.inventory.armorInventory) {
                ...
                if (percent <= this.armorThreshhold.getValue() && !this.entityArmorArraylist.containsKey(player)) {
                    if (player == ArmorMessage.mc.player && this.notifySelf.getValue().booleanValue()) {
                        Command.sendMessage(player.getName() + " laooo " + this.getArmorPieceName(stack) + " cani azaliyooorr!", this.notification.getValue());
                    } else {
                        ArmorMessage.mc.player.sendChatMessage("/msg " + player.getName() + " " + "Aloo" + player.getName() + this.getArmorPieceName(stack) + " cani azaliyor lao!");
                    }

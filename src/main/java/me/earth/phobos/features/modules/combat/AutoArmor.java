...
public class AutoArmor
    ...
    public void onTick() {
        ...
        if (this.taskList.isEmpty()) {
            ...
            ItemStack chest;
            int slot4;
            if (this.mendingTakeOff.getValue().booleanValue() && InventoryUtil.holdingItem(ItemExpBottle.class) && AutoArmor.mc.gameSettings.keyBindUseItem.isKeyDown() && (this.isSafe() || EntityUtil.isSafe(AutoArmor.mc.player, 1, false))) {
                int bootDamage;
                int leggingDamage;
                int chestDamage;
                int helmDamage;
        ...
        if (this.timer.passedMs((int) ((float) this.delay.getValue().intValue() * (this.tps.getValue() != false ? Agalar.serverManager.getTpsFactor() : 1.0f)))) {
            if (!this.taskList.isEmpty()) {

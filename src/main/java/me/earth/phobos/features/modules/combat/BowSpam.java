...
public class BowSpam extends Module {
    ...
    public void onUpdateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        ...
        if (autoSwitch.getValue() && InventoryUtil.findHotbarBlock(ItemBow.class) != -1 && ownHealth.getValue() <= EntityUtil.getHealth(mc.player) && (!onlyWhenSave.getValue() || EntityUtil.isSafe(mc.player))) {
            ...
            if (target != null) {
                AutoCrystal crystal = Agalar.moduleManager.getModuleByClass(AutoCrystal.class);
        ...
        if (mode.getValue() == Mode.FAST) {
            if (offhand || mc.player.inventory.getCurrentItem().getItem() instanceof ItemBow) {
                if (mc.player.isHandActive() && mc.player.getItemInUseMaxCount() >= ticks.getValue() * (tpsSync.getValue() ? Agalar.serverManager.getTpsFactor() : 1)) {
    ...
    public void onUpdate() {
        ...
        switch (mode.getValue()) {
            case AUTORELEASE:
                if (offhand || mc.player.inventory.getCurrentItem().getItem() instanceof ItemBow) {
                    if (timer.passedMs((int) (delay.getValue() * (tpsSync.getValue() ? Agalar.serverManager.getTpsFactor() : 1)))) {

...
public class HoleFiller
    ...
    public void onDisable() {
        if (this.offhand.getValue().booleanValue()) {
            Agalar.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode);
            Agalar.moduleManager.getModuleByClass(Offhand.class).setMode(this.offhandMode2);
        }
    ...
    private void doHoleFill() {
        ...
        if (this.midSafeHoles.getValue().booleanValue()) {
            object = Agalar.holeManager.getMidSafety();
            synchronized (object) {
                targets = new ArrayList<BlockPos>(Agalar.holeManager.getMidSafety());
            }
        }
        object = Agalar.holeManager.getHoles();
        synchronized (object) {
            targets = new ArrayList<BlockPos>(Agalar.holeManager.getHoles());
        }
    ...
    private boolean check() {
        ...
        this.switchItem(true);
        if (!this.freecam.getValue().booleanValue() && Agalar.moduleManager.isModuleEnabled(Freecam.class)) {
            return true;
        }

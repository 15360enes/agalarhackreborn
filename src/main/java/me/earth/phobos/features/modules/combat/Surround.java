...
public class Surround
    ...
    public void onEnable() {
        ...
        if (this.center.getValue().booleanValue() && !Agalar.moduleManager.isModuleEnabled("Freecam")) {
    ...
    private void doFeetPlace() {
        ...
        if (!EntityUtil.isSafe(Surround.mc.player, 0, this.floor.getValue())) {
            this.isSafe = 0;
            this.placeBlocks(Surround.mc.player.getPositionVector(), EntityUtil.getUnsafeBlockArray(Surround.mc.player, 0, this.floor.getValue()), this.helpingBlocks.getValue(), false, false);
        ...
        for (Vec3d vec3d : vec3ds) {
            ...
            switch (BlockUtil.isPositionPlaceable(position, this.raytrace.getValue())) {
                ...
                case 1: {
                    ...
                    if (!this.extendMove.getValue().booleanValue() && Agalar.speedManager.getSpeedKpH() != 0.0 || isExtending || this.extenders >= this.extender.getValue())
                        continue block6;
    ...
    private boolean check() {
        ...
        return Agalar.moduleManager.isModuleEnabled("Freecam") || !this.timer.passedMs(this.delay.getValue().intValue()) || this.switchMode.getValue() == InventoryUtil.Switch.NONE && Surround.mc.player.inventory.currentItem != InventoryUtil.findHotbarBlock(BlockObsidian.class);
    }

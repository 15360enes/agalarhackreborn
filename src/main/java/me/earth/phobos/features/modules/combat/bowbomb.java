...
public class bowbomb extends Module {
    ...
    public bowbomb() { super("FastProjectile", "Uno hitter w bows", Category.MISC, true, false, false); }
    ...
    public Setting <Integer> Timeout = this.register ( new Setting <> ( "Timeout", 5000, 10, 20000 ) );
    public Setting <Integer> spoofs = this.register ( new Setting <> ( "Spoofs", 10, 1, 400 ) );
        ...
        if (event.getPacket() instanceof CPacketPlayerDigging) {
            ...
            if (packet.getAction() == CPacketPlayerDigging.Action.RELEASE_USE_ITEM) {
                ...
                if (!handStack.isEmpty() && handStack.getItem() != null && handStack.getItem() instanceof ItemBow && Bows.getValue()) {
                    doSpoofs();
                    if (debug.getValue()) Command.sendMessage("About to cum");
                }
            }
    ...
    private void doSpoofs() {
        if (System.currentTimeMillis() - lastShootTime >= Timeout.getValue()) {
            ...
            for (int index = 0; index < spoofs.getValue(); ++index) {
                //spleefNet.positionHandler.offsetPositionPacket(new Vec3d(0, -1e-6, 0), true);
                //spleefNet.positionHandler.offsetPositionPacket(new Vec3d(0, 1e-6, 0), false);
                //Phobos.positionManager.offsetPlayerPos(new Vec3d(0, -1e-6, 0));
                //Phobos.positionManager.offsetPlayerPos(new Vec3d(0, 1e-6, 0));
                if (bypass.getValue()) {
            ...
            if (debug.getValue()) Command.sendMessage("You came");

...
public class LongJump
        ...
        switch (this.mode.getValue()) {
            ...
            case DIRECT: {
                ...
                if (!LongJump.mc.player.collidedVertically) {
                    ...
                    Agalar.timerManager.setTimer(0.85f);
    ...
    private void doVirtue(MoveEvent event) {
        if (this.mode.getValue() == Mode.VIRTUE && (LongJump.mc.player.moveForward != 0.0f || LongJump.mc.player.moveStrafing != 0.0f && !EntityUtil.isOnLiquid() && !EntityUtil.isInLiquid())) {
            ...
            List collidingList = LongJump.mc.world.getCollisionBoxes(LongJump.mc.player, LongJump.mc.player.getEntityBoundingBox().offset(0.0, LongJump.mc.player.motionY, 0.0));
            List collidingList2 = LongJump.mc.world.getCollisionBoxes(LongJump.mc.player, LongJump.mc.player.getEntityBoundingBox().offset(0.0, -0.4, 0.0));

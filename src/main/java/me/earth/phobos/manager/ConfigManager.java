...
class ConfigManager
    ...
    void setValueFromJson ( Feature feature , Setting setting , JsonElement element ) {
        switch (setting.getType ( )) {
            ...
            default: {
                Agalar.LOGGER.error ( "Unknown Setting type for: " + feature.getName ( ) + " : " + setting.getName ( ) );
            }
        }
    }
    ...
    void loadFile ( JsonObject input , Feature feature ) {
        for (Map.Entry < String, JsonElement > entry : input.entrySet ( )) {
            ...
            if ( feature instanceof FriendManager ) {
                try {
                    Agalar.friendManager.addFriend ( new FriendManager.Friend ( element.getAsString ( ) , UUID.fromString ( settingName ) ) );
                } catch ( Exception e ) {
                    e.printStackTrace ( );
                }
            } else {
    ...
    void saveConfig ( String name ) {
        ...
        Agalar.friendManager.saveFriends ( );
        for (Feature feature : this.features) {
            try {
    ...
    public
    void init ( ) {
        this.features.addAll ( Agalar.moduleManager.modules );
        this.features.add ( Agalar.friendManager );
        String name = this.loadCurrentConfig ( );
        this.loadConfig ( name );
        Agalar.LOGGER.info ( "Config loaded." );
    }
    ...
    void loadPath ( Path path , Feature feature ) throws IOException {
        ...
        try {
            ...
            Agalar.LOGGER.error ( "Bad Config File for: " + feature.getName ( ) + ". Resetting..." );

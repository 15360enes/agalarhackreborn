...
public class TotemPopManager
    ...
    public void onUpdate() {
        if (this.notifications.totemAnnounce.passedMs(this.notifications.delay.getValue().intValue()) && this.notifications.isOn() && this.notifications.totemPops.getValue().booleanValue()) {
            for (EntityPlayer player : this.toAnnounce) {
                ...
                Command.sendOverwriteMessage("\u00a7c" + player.getName() + " popped " + "\u00a7l" + this.getTotemPops(player) + "\u00a7c" + " Totem" + (this.getTotemPops(player) == 1 ? "" : "s") + ".", playerNumber, this.notifications.totemNoti.getValue());
    ...
    public void init() {
        this.notifications = Agalar.moduleManager.getModuleByClass(Notifications.class);
    }
    ...
    public void onDeath(EntityPlayer player) {
        if (this.getTotemPops(player) != 0 && !player.equals(TotemPopManager.mc.player) && this.notifications.isOn() && this.notifications.totemPops.getValue().booleanValue()) {
            ...
            Command.sendOverwriteMessage("\u00a7c" + player.getName() + " died after popping " + "\u00a7l" + this.getTotemPops(player) + "\u00a7c" + " Totem" + (this.getTotemPops(player) == 1 ? "" : "s") + ".", playerNumber, this.notifications.totemNoti.getValue());
            this.toAnnounce.remove(player);
        }
    ...
    public void onLogout(EntityPlayer player) {
        if (this.getTotemPops(player) != 0 && !player.equals(TotemPopManager.mc.player) && this.notifications.isOn() && this.notifications.totemPops.getValue().booleanValue()) {
            ...
            Command.sendOverwriteMessage("\u00a7c" + player.getName() + " logged after popping " + "\u00a7l" + this.getTotemPops(player) + "\u00a7c" + " Totem" + (this.getTotemPops(player) == 1 ? "" : "s") + ".", playerNumber, this.notifications.totemNoti.getValue());
            this.toAnnounce.remove(player);
        }
            this.resetPops(player);
        }

    public void onOwnLogout(boolean OnLogout) {
        if (OnLogout)  {
            this.clearList();
        }
    }

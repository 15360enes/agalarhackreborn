...
import me.earth.phobos.features.modules.render.ShaderChams;
...
public abstract class Shader
{
    ...
    public Shader(final String fragmentShader) {
        ...
        try {
            final InputStream vertexStream = this.getClass().getResourceAsStream("/assets/Phobos/shader/vertex.vert");
            vertexShaderID = this.createShader(IOUtils.toString(vertexStream, Charset.defaultCharset()), 35633);
            IOUtils.closeQuietly(vertexStream);
            final InputStream fragmentStream = (ShaderChams.getInstance().mode.getEnum().equals(ShaderChams.modes.Custom) ? inputStream : this.getClass().getResourceAsStream("/assets/phobos/shader/" + fragmentShader));

...
public class EntityUtil
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtil.isDead(entity) || entity.equals(EntityUtil.mc.player) || entity instanceof EntityPlayer && Agalar.friendManager.isFriend(entity.getName()) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && Agalar.friendManager.isFriend((EntityPlayer) entity)) {
                color = new Color(0.33333334f, 1.0f, 1.0f, (float) alpha / 255.0f);
            }
            Killaura killaura = Agalar.moduleManager.getModuleByClass(Killaura.class);
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            if (player.isInvisible() && !Managers.getInstance().tRadarInv.getValue().booleanValue() || player.getName().equals(EntityUtil.mc.player.getName()))
                continue;
            ...
            output.put(healthSB.toString() + " " + (Agalar.friendManager.isFriend(player) ? "\u00a7b" : "\u00a7r") + player.getName() + " " + distanceSB.toString() + " " + "\u00a7f" + Agalar.totemPopManager.getTotemPopString(player) + Agalar.potionManager.getTextRadarPotion(player), (int) EntityUtil.mc.player.getDistance(player));

...
class EntityUtil implements Util {
    ...
    List < EntityPlayer > getNearbyPlayers ( double d ) {
        ...
        list.removeIf ( entityPlayer -> Agalar.friendManager.isFriend ( entityPlayer.getName ( ) ) );
        return list;
    }
    ...
    boolean isntValid ( final Entity entity , final double range ) {
        return entity == null || isDead ( entity ) || entity.equals ( EntityUtil.mc.player ) || ( entity instanceof EntityPlayer && Agalar.friendManager.isFriend ( entity.getName ( ) ) ) || EntityUtil.mc.player.getDistanceSq ( entity ) > MathUtil.square ( range );
    }
    ...
    Color getColor ( final Entity entity , final int red , final int green , final int blue , final int alpha , final boolean colorFriends ) {
        ...
        if ( entity instanceof EntityPlayer ) {
            ...
            final Killaura killaura = Agalar.moduleManager.getModuleByClass ( Killaura.class );
    ...
    Map < String, Integer > getTextRadarPlayers ( ) {
        ...
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            output.put ( healthSB + " " + ( Agalar.friendManager.isFriend ( player ) ? "\u00A7b" : "\u00A7r" ) + player.getName ( ) + " " + distanceSB + " " + "\u00A7f" + Agalar.totemPopManager.getTotemPopString ( player ) + Agalar.potionManager.getTextRadarPotion ( player ) , (int) EntityUtil.mc.player.getDistance ( player ) );
